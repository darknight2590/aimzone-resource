...
function revivePlayer(closestPlayer)
	...
	ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
		if quantity > 0 then
			...
			if IsEntityPlayingAnim(closestPlayerPed, 'misslamar1dead_body', 'dead_idle', 3) then --if IsPedDeadOrDying(closestPlayerPed, 1) then
				local playerPed = PlayerPedId()
				local lib, anim = 'mini@cpr@char_a@cpr_str', 'cpr_pumpchest'
				TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('revive_inprogress')})
				...
				TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('player_not_unconscious')})
			end
		else
			TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('not_enough_medikit')})
		end
		isBusy = false
	end, 'medikit')
end
...
AddEventHandler('esx_ambulancejob:heal', function(healType, quiet)
	...
	if not quiet then
		TriggerEvent('mythic_notify:client:SendAlert', { type = 'success', text = _U('healed')})
	end
end)
...
AddEventHandler('medic:emsRevive', function()
	...
	if closestPlayer == -1 or closestDistance > 1.0 then
		TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('no_players')})
	else
		revivePlayer(closestPlayer)
	end
end)
		...
		ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
			if quantity > 0 then
				...
				if health > 0 then
					...
					isBusy = true
					TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('heal_inprogress')})
					TaskStartScenarioInPlace(playerPed, 'CODE_HUMAN_MEDIC_TEND_TO_DEAD', 0, true)
					Citizen.Wait(10000)
					ClearPedTasks(playerPed)
					...
					TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('player_not_conscious')})
				end
			else
				TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('not_enough_bandage')})
			end
		end, 'bandage')
	end
end)
	...
	if player ~= -1 and distance <= 3.0 then
		ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', { title = 'Fatura kes'}, function(data, menu)
			local amount = tonumber(data.value)
				...
				if amount == nil then
					TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Hatalı Miktar'})
				else
